...
//npx hardhat run scripts/approveTransaction.js --network harmony_mainnet
//npx hardhat run scripts/approveTransaction.js --network mainnet


//******Update the transaction id before confirming */
var transaction_id = 1;



async function approveTx(net) {
    try {
    ...
    try{
      ...
      //now how do I use this 
        let tx = await multisigSigner.confirmTransaction(transaction_id);
...
approveTx("harmony_mainnet", process.env.MAINNET_PK, process.env.NODE_URL_HARMONY_MAINNET)
//approveTx("mainnet", process.env.MAINNET_PK, process.env.NODE_URL_MAINNET)
.then(() => process.exit(0))
.catch(error => {
console.error(error);
process.exit(1);
